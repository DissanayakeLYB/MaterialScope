...
const crystal_structures_1 = require("../lib/crystal-structures");
...
function replicate(ref) {
    const atoms = [];
    for (let i = -1; i <= 1; i++) {
        for (let j = -1; j <= 1; j++) {
            for (let k = -1; k <= 1; k++) {
                const shift = add(add(scale(ref.vectors[0], i), scale(ref.vectors[1], j)), scale(ref.vectors[2], k));
...
function neighborhood(ref, radius) {
    const seen = new Map();
    for (const atom of replicate(ref)) {
        if (dist(atom, [0, 0, 0]) <= radius)
...
function check(label, actual, expected, tol = 1e-3) {
    ...
    if (!ok)
        failures++;
    console.log(`${ok ? "PASS" : "FAIL"}  ${label.padEnd(52)} got ${actual.toFixed(6)}  want ${expected.toFixed(6)}`);
}
function checkEq(label, actual, expected, tol = 1e-9) {
    const ok = Math.abs(actual - expected) <= tol;
    if (!ok)
        failures++;
    console.log(`${ok ? "PASS" : "FAIL"}  ${label.padEnd(52)} got ${actual.toFixed(6)}  want ${expected.toFixed(6)}`);
}
for (const type of Object.keys(REFERENCE)) {
    const ref = REFERENCE[type];
    const config = (0, crystal_structures_1.getStructureConfig)(type);
    console.log(`\n=== ${type.toUpperCase()} ===`);
    if (!config) {
        failures++;
    ...
    for (const origin of ref.basis) {
        ...
        for (const atom of replicate(ref)) {
            ...
            if (d < 1e-9)
                continue;
            if (Math.abs(d - ref.nnDistance) <= tol)
                count++;
        }
        counts.push(count);
    }
    const independentCn = counts[0];
    checkEq(`independent coordination number (${counts.join(",")})`, independentCn, ref.coordinationNumber);
    checkEq(`lib coordination number agrees`, config.coordinationNumber, ref.coordinationNumber);
    ...
    const candidateOffsets = type === "hcp"
        ? [[0, 0, HCP_C / 2]]
        : [
            [0, 0, 0],
            [0.5, 0.5, 0.5],
    ...
    for (const offset of candidateOffsets) {
        const orphans = config.sites.filter((site) => {
            ...
            return !latticeAtoms.some((atom) => dist(atom, shifted) <= 1e-6);
        });
        if (orphans.length === 0) {
            offsetFound = true;
            console.log(`PASS  all ${config.sites.length} displayed sites are lattice atoms (offset ${offset.map((v) => v.toFixed(2)).join(",")})`);
            break;
        }
    }
    if (!offsetFound) {
    ...
    if (detached.length === 0) {
        ...
        failures++;
        console.log(`FAIL  sites with no touching neighbor in display:`, detached.map((s) => s.position));
    }
}
// HCP c/a ratio must be the ideal √(8/3).
const hcp = (0, crystal_structures_1.getStructureConfig)("hcp");
checkEq(`hcp c/a = √(8/3)`, hcp.cOverA ?? 0, Math.sqrt(8 / 3));
checkEq(`hcp cell volume = (3√3/2)a²c`, hcp.cellVolume, (3 * Math.sqrt(3) / 2) * HCP_C, 1e-9);
